...
- It is not appropriate because it can predict values outside of the reasonable range of dvote with a linear model. 
...
- divide by 4 rule gives you an upper bound on the effect of a one unit change of that variable.
- Changing gender (female to male) would give you a `-.159/4 change in dvote. 
- Being in an income bracket that is one level higher, dvote would be -.288/4 lower for the person in the higher bracket than it would be for the person in the lower bracket. 
...
Bayesian would be that we have 95% confidence that the ture coefficient for income falls inbetween our confidence intervals, and the frequentist would be that if we conducted this infinite amount of times the coefficient we find would fall in our range 95% of the time. 
...
As you increase your income, you're less likely to prefer the democratic candidate. 
...
```{r}
model_1 %>%
  summarize(emplogis = qlogis(coefficients))
```
...
- predicted value based on the model (regression line given an x value)
...
Its on a simplified model, so it makes sense that everything would have to fall on that line and have the same outcome. 
